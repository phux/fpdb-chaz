...
class DerivedStats():
    ...
    def assembleHandsStove(self, hand):
        ...
        for player in hand.players:
            hole, cards, bcards = [], [], []
            best_lo, locards, lostring, lo_id = None, None, None, 1
            best_hi, hicards, histring, hi_id = None, None, None, 1
            pname = player[1]
            ...
            if (hp['sawShowdown']) or hero:
                if category not in ('badugi', 'razz', '2_holdem', '5_omahahi'):
                    ...
                    for street, board in boards.iteritems():
                        ...
                        if (board['allin'] or (hero and streetSeen) or hp['sawShowdown']): #(hero and streetSeen) or hp['sawShowdown']):
                            boardId = 0
                            for n in range(len(board['board'])):
                                ...
                                histring, lostring, histringold, lostringold, lostringvalue, histringvalue, winnings, hi_id, lo_id = None, None, None, None, 0, 0, 0, 1, 1
                                ...
                                if (u'0x' not in cards) and ((base=='hold' and len(board['board'][n])>=3) or (base!='hold' and len(cards)==reduce(lambda x, y: y-x,hrange))):
                                     ...
                                     if lostring:
                                         lostring = self.getCards('lo', lostring, locards, best_lo)

                                     if histring:
                                         histring = self.getCards('hi', histring, hicards, best_hi)

                                self.handsstove.append( [hand.dbid_hands,
        ...
        #self.handsstove += [t[:8] + [0] for t in inserts_temp]
        startstreet = None
